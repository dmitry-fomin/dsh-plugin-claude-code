...
- **Модель не та, что нужна.** Разово она переключается флагами `--model` и `--provider`
  при делегировании — файл `~/.dsh/settings.yaml` для этого править не надо. Постоянную
  смену делает человек сам в настройках харнесса. Алиасы `pro|flash|vision` работают
  только на маршруте `deepseek-official`; на чужом маршруте нужен полный id модели.
