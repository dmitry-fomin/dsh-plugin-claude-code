---
name: dsh-delegate
description: Отдать задачу DeepSeek Harness (dsh) — второму агентному харнессу, который сам ходит по коду в рабочем каталоге. Запускается фоновой задачей со своим идентификатором, и ты продолжаешь работать, пока dsh думает. Используй, когда нужно обойти незнакомую кодовую базу или подсистему целиком, собрать карту или найти все вхождения, получить разбор от другой модели с доступом к файлам, или когда человек просит «отдай дипсику», «спроси dsh», «пусть deepseek посмотрит». По умолчанию dsh работает только на чтение.
when_to_use: Триггер-фразы — «делегируй dsh», «отдай задачу дипсику», «пусть dsh разберётся», «спроси у dsh», «запусти дипсика в фоне». Явная просьба человека = согласие на запуск. Годится и без упоминания dsh, когда надо обойти незнакомую базу целиком, а тащить её в свой контекст дорого. Не бери задачи на изменение кода, не адресованные dsh явно, мелочь, на которую быстрее ответить самому, и вопросы синтаксиса или API. Если гипотеза уже есть и нужна проверка — это /dsh:dsh-second-opinion. Управление уже запущенными задачами — /dsh:dsh-jobs.
argument-hint: "[--write] [--model pro|flash] [--sync] [что должен сделать dsh]"
...
1. **Запусти.** Один вызов Bash — он возвращает идентификатор задачи одной строкой:
...
3. **Поставь ожидание фоновым вызовом Bash** (`run_in_background: true`), чтобы тебя
   ...
   Уведомление о завершении этого вызова и есть сигнал «dsh закончил».
