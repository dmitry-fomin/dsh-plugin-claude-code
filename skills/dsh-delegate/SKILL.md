...
| названы другой провайдер или маршрут («через openrouter», «спроси glm») | `--provider <маршрут>` и полный id в `--model` |
...
Без указаний модель не выбирай — сработает выбор пользователя из настроек, включая
провайдера. Алиасы `pro|flash|vision` живут только на нативном маршруте
`deepseek-official`: если человек настроил другой провайдер, скрипт отвергнет алиас и
попросит полный id модели — это не поломка, а защита от тихой подмены маршрута.
