---
name: dsh-runtime
description: Используй когда нужно вызвать DeepSeek Harness из Claude Code — контракт скрипта dsh-run.sh, фоновые задачи и их идентификаторы, режимы прав, выбор модели, таймауты и коды возврата. Внутренний справочник, подключается к субагенту dsh-runner.
...
теряются режим прав по умолчанию, выбор модели, учёт фоновых задач и разбор кодов
возврата.
...
| `--model pro\|flash\|vision\|<id>` | модель из настроек пользователя | `pro` — сложное рассуждение, `flash` — быстрые механические задачи; алиасы — каталог DeepSeek, на чужом маршруте нужен полный id |
| `--effort low\|medium\|high\|xhigh\|max` | из настроек | можно и без `--model`: модель тогда остаётся выбранной пользователем |
| `--provider <маршрут>` | провайдер из настроек, иначе `deepseek-official` | сменить маршрут на один прогон: `openrouter`, `zai`, свой шлюз |
...
Активную модель и поднятые маршруты показывает `check`. Учти особенность: **`--patch` сам
по себе модель не меняет** — пользовательский слой `~/.dsh/settings.yaml` накладывается
поверх любого overlay. Скрипт обходит это, подменяя путь, из которого читается документ
настроек; поэтому меняй модель только через `--model`, а не своими патчами.

Раз подменяется путь, overlay несёт **весь** документ настроек: скрипт копирует
`~/.dsh/settings.yaml` и переписывает в копии одну секцию `agent-default-model:`. Секции
`llm-deepseek:` и `llm-pi-ai:` (ссылка на ключ и профили провайдеров) остаются на месте —
без этого прогон с `--model` молча уезжал бы на дефолтный ключ и нативный маршрут.

Провайдер берётся из настроек пользователя; `--provider` меняет его на один прогон.
Нативный маршрут DeepSeek — `deepseek-official`, его каталог: `deepseek-v4-pro` (`pro`),
`deepseek-v4-flash` (`flash`), `deepseek-v4-flash-vision-exp` (`vision`). На любом другом
маршруте (`openrouter`, `zai`, свой OpenAI-совместимый шлюз) этих id нет: алиас там
отвергается с подсказкой, модель называй полным id (`deepseek/deepseek-v4-pro`,
`google/gemini-3.1-pro-preview`, `glm-5.2`). Постоянные алиасы под свой маршрут задаются
переменными `DSH_MODEL_PRO`, `DSH_MODEL_FLASH`, `DSH_MODEL_VISION`.
