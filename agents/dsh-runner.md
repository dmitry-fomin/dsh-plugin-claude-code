...
| назван другой провайдер или маршрут | `--provider <маршрут>` плюс полный id в `--model` |
...
правки. Модель без указаний оставляй по умолчанию. Алиасы `pro|flash|vision` действуют только на
маршруте `deepseek-official`; на другом провайдере скрипт их отвергнет и попросит полный
id модели.
